...
final_output_validator_agent=base_agent_obj.create_assistant_agent('final_output_validator_agent',prompt_obj.final_output_validator_agent_prompt,tools=[fetch_drug],reflect_on_tool_use=True,description='Handles validating final json prescrition with database.')
...
async def workflow(input_content):
    print('----------------User Input---------------------')
    print(input_content)
    ...
    while True:
        if input_valid_flag==None:
            result1=await input_validator_agent.run(task=input_content,cancellation_token=CancellationToken())
            result1=result1.messages[-1].content
            print('----------------Input Validator---------------------')
            print(result1)
            #input_validator_agent.on_reset()
        
        if not 'INVALID' in result1.upper() or input_valid_flag:
            input_valid_flag=True
            result2=await prescription_extractor_agent.run(task=input_content,cancellation_token=CancellationToken())
        ...
        if json_written_flag:
            result3=await final_output_validator_agent.run(task=input_content,cancellation_token=CancellationToken())
